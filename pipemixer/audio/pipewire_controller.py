...
class PipeWireController:
    ...
    def save_bindings(self, slider_app: list[str | None]) -> None:
        try:
            with open(_get_save_path(), "w") as f:
                json.dump({"bindings": slider_app}, f, indent=2)
        except Exception as e:
            print(f"Failed to save bindings: {e}")

    def load_bindings(self, num_channels: int) -> list[str | None]:
        try:
            with open(_get_save_path()) as f:
                data = json.load(f)
            bindings = data.get("bindings", [None] * num_channels)
            while len(bindings) < num_channels:
                bindings.append(None)
            return bindings[:num_channels]
        except FileNotFoundError:
    ...
    def _get_process_tree(self, pid: str) -> set[str]:
        """Return all PIDs in the same Steam/Wine process group as pid."""
        pids = {pid}
        try:
            current = pid
            ancestors = []
    ...
    def _match_process_tree_in_pipewire(self, pid: str) -> str | None:
        """
        Find a PipeWire sink input whose client process is in the same
        process tree as pid. Handles Wine/Proton games.
        """
    ...
    def _get_focused_app_by_name(self, window_id: str) -> str | None:
        ...
        # 2. Normalized match (strips .exe for Proton games)
        for app in pipewire_apps:
            norm = normalize(app)
            if norm == normalize(class_name) or norm == normalize(window_title):
                return app

        # 3. Substring match against wpctl nodes
        for app in pipewire_apps:
            norm = normalize(app)
        ...
        # 4. Match against pactl application.name / node.name
        for inp in self._get_sink_inputs():
    ...
    def _find_all_sink_input_ids(self, app_name: str) -> list[str]:
        """Return all pactl sink input IDs matching app_name."""
        name = app_name.lower()
        ...
        # Pass 2: wpctl node ID -> pactl client.id (handles Spotify)
    ...
    def _apply_volume_by_name(self, app_name: str, volume: float) -> None:
        """Set volume on all matching sink inputs (handles apps with multiple streams)."""
        ...
        for sink_id in sink_ids:
            subprocess.run(
                ["pactl", "set-sink-input-volume", sink_id, percent],
